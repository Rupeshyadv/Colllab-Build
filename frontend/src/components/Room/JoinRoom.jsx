...
const JoinRoom = () => {
  const [roomDest, setRoomDest] = useState('')  // sesssionId/language 

  const navigate = useNavigate()  
  const handleJoinByRoomId = async () => {
    const [roomId, language] = roomDest.split('/')
    if (roomId.trim()) {
        const data = await joinSessionRoom(roomId)

        navigate(`/room/${roomDest}`)
    }
  };
            ...
            <div className="flex justify-between items-center h-16">
                <div className="flex items-center space-x-4">
                    <input
                        ...
                        value={roomDest}
                        onChange={(e) => setRoomDest(e.target.value)}
